...
대시보드는 여러 화면에서 확인한 데이터를 한곳에 요약해서 보여준다. 처음에는 차트 코드를 보기 전에 각 지표가 어떤 테이블에서 계산되는지 확인한다.

1. 품목 수, LOT 수, 생산실적 수가 어떤 테이블의 행 수인지 확인한다.
2. 일자별 생산량이 `production.production_date`와 `production.qty`에서 계산되는지 확인한다.
3. 품목별 생산량이 `production`과 `item`을 연결해서 계산되는지 확인한다.
4. LOT별 사용 횟수가 `production_material`을 기준으로 계산되는지 확인한다.

대시보드는 새 데이터를 입력하는 화면이 아니라, 기존 데이터를 요약해서 질문에 답하는 화면이다.
...
처음 배우는 학생은 KPI를 "큰 숫자로 보여주는 요약값"으로 이해하면 된다. 표를 읽기 전에 현재 DB에 데이터가 얼마나 있는지 빠르게 확인하는 용도다.
...
표는 정확한 숫자를 확인하기 좋고, 차트는 날짜별 변화 방향을 빠르게 보기 좋다. 같은 데이터를 두 방식으로 보여주는 이유는 사용 목적이 다르기 때문이다.
...
초보자는 먼저 "DB에서 가져온 결과도 Python에서 다시 정렬하거나 일부만 자를 수 있다"는 점을 이해하면 된다. 다만 데이터가 아주 많아지면 SQL에서 필요한 만큼만 조회하는 방식이 더 적합하다.
...
이 원칙은 교육 프로젝트에서 특히 중요하다. 화면에 멋진 지표를 추가하는 것보다, 실제 테이블과 컬럼에서 어떻게 계산되는지 설명할 수 있는 지표만 만드는 것이 더 중요하다.
...
```text
1. src/queries.py에 total_lot_qty() 같은 조회 함수 추가
2. pages/08_생산현황.py에서 함수 호출
3. metric_row()에 새 KPI 추가
4. 화면에서 숫자가 표시되는지 확인
```
...
```text
집계 SQL 작성 -> src/queries.py
차트와 표 표시 -> pages/08_생산현황.py
```
...
이 과제는 생산 등록 화면과 대시보드가 같은 DB를 보고 있다는 점을 확인하는 실습이다. 저장 후 새로고침하거나 다른 화면으로 이동하면 집계 결과가 바뀌어야 한다.
