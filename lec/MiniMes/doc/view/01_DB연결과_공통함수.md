...
이 문서는 화면보다 공통 코드를 먼저 다룬다. 초보자는 함수 이름을 모두 외우기보다 "화면이 DB를 직접 만지지 않고 공통 함수를 거친다"는 점을 먼저 이해하면 된다.

1. `src/db.py`를 열어 `DB_PATH`가 어디를 가리키는지 확인한다.
2. `src/ui.py`를 열어 `page_title()`, `show_dataframe()` 함수 이름을 확인한다.
3. 품목 조회 화면을 열고 `queries.items()`처럼 공통 조회 함수를 호출하는 부분을 찾는다.

수업에서는 화면 하나를 실행해 본 뒤, 그 화면이 어떤 공통 함수를 사용하는지 거꾸로 따라가면 이해하기 쉽다.
...
처음에는 `PRAGMA foreign_keys = ON`이 낯설 수 있다. 이 설정은 예를 들어 존재하지 않는 품목 ID로 LOT를 저장하는 실수를 막기 위해 필요하다. MES 데이터는 품목, LOT, 생산실적이 서로 연결되므로 관계 검사가 중요하다.
...
예를 들어 품목 조회 화면에서 `queries.items()`를 호출하면 내부에서는 다음처럼 이어진다.

```text
pages/01_품목_조회.py
-> src/queries.py의 items()
-> src/db.py의 fetch_dataframe()
-> sql/mini_mes.db
-> 화면의 표
```

이 구조 덕분에 화면 파일은 검색 조건과 표시 방법에 집중하고, SQL은 `queries.py`에서 따로 읽을 수 있다.
...
처음 배우는 학생은 공통 UI 함수를 "복사 붙여넣기를 줄이는 도구"로 이해하면 된다. 모든 화면에서 제목 모양과 빈 결과 안내가 같아지면, 수업 중에는 기능 차이에만 집중할 수 있다.
...
```text
DB 파일 위치 확인 -> src/db.py
DB 연결 오류 메시지 수정 -> src/db.py
화면 상단 학습 안내 형식 수정 -> src/ui.py
표가 비었을 때 메시지 수정 -> src/ui.py
```
