...
처음에는 코드를 바로 수정하지 말고 앱을 실행해서 화면 구조를 먼저 확인한다. 화면이 어떻게 나뉘어 있는지 알아야 뒤에서 파일과 코드를 찾기 쉽다.

1. `streamlit run app.py`로 앱을 실행한다.
2. 왼쪽 메뉴에 어떤 화면이 있는지 확인한다.
3. 소개 화면에서 DB 연결 상태와 테이블 목록을 확인한다.
4. `pages/` 폴더의 파일명과 왼쪽 메뉴의 화면 이름을 비교한다.

이 단계의 목표는 기능을 모두 이해하는 것이 아니라, "화면 하나가 Python 파일 하나와 연결된다"는 구조를 익히는 것이다.
...
```text
app.py 실행
-> setup_page()로 Streamlit 페이지 설정
-> page_title()로 학습 안내 표시
-> show_database_status()로 DB 파일 확인
-> queries.table_counts()로 주요 테이블 행 수 조회
-> metric_row()와 show_dataframe()으로 결과 표시
```

초보자는 이 흐름에서 두 가지만 먼저 기억하면 된다.

- 화면에 직접 보이는 코드는 `app.py`와 `pages/`에 있다.
- DB에서 데이터를 읽어 오는 코드는 대부분 `src/queries.py`에 있다.
...
```text
화면 제목이나 안내 문구를 바꾸고 싶다 -> app.py 또는 pages/*.py
DB 조회 SQL을 바꾸고 싶다 -> src/queries.py
공통 표 출력 방식을 바꾸고 싶다 -> src/ui.py
```
