...
생산 등록 화면은 조회 화면과 달리 DB에 새 데이터를 저장한다. 처음에는 저장 버튼을 누르기 전에 화면이 어떤 값을 요구하는지 순서대로 확인한다.

1. 생산할 완제품 품목을 선택한다.
2. 생산번호와 새 완제품 LOT 번호를 확인한다.
3. 생산수량과 유효기한을 입력한다.
4. 투입할 원자재 LOT를 하나 이상 선택한다.
5. 각 원자재 LOT의 투입수량을 확인한다.
6. 저장 후 생산실적 조회, 생산이력, 역방향 추적 화면에서 새 데이터가 보이는지 확인한다.

이 화면의 핵심은 "한 번의 저장으로 여러 테이블에 데이터가 같이 들어간다"는 점이다.
...
선택박스는 고정 목록을 코드에 적어 둔 것이 아니라 DB에서 읽어 온 데이터로 만든다. 그래서 DB에 품목이나 LOT가 추가되면 화면 선택 목록도 함께 바뀐다.
...
처음 배우는 학생은 `st.form()`을 "저장 버튼을 누르기 전까지 입력값을 모아 두는 상자"로 이해하면 된다. 생산 등록은 생산번호, 완제품 LOT, 원자재 LOT 여러 개를 한 번에 저장해야 하므로 폼으로 묶는 것이 적합하다.
...
```text
Streamlit 입력값
-> ProductionRegistration 객체
-> validate_registration() 검증
-> register_production() 저장
-> lot INSERT
-> production INSERT
-> production_material INSERT 여러 건
```

초보자는 먼저 이 순서를 읽고, 그 다음 `pages/04_생산_등록.py`와 `src/services.py`를 번갈아 보면 된다.
...
검증은 사용자가 잘못 입력했을 때 DB에 이상한 데이터가 들어가지 않도록 막는 단계다. 예를 들어 원자재 LOT를 선택하지 않고 저장하면 생산실적은 있는데 투입 원자재가 없는 데이터가 생길 수 있으므로 저장을 막는다.
...
트랜잭션은 "모두 성공하거나 모두 실패해야 하는 작업 묶음"이다. 완제품 LOT만 생성되고 생산실적 저장이 실패하면 데이터 관계가 깨진다. 그래서 생산 등록에서는 세 테이블 INSERT를 하나의 트랜잭션으로 처리한다.
...
```text
입력 위젯 수정 -> pages/04_생산_등록.py
검증 조건 수정 -> src/services.py의 validate_registration()
INSERT 컬럼 수정 -> src/services.py의 register_production()
선택 목록 SQL 수정 -> src/queries.py
```
